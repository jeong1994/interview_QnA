...
th:test, th:if, th:each 같은 속성을 사용해 구조를 깨지 않고 동적 처리를 할 수 있습니다`
  },
  {
    ...
    answer: `오버로딩은 같은 이름의 메서드를 매개변수만 다르게 해서 여러 개 정의하는 것이고,<br>
오버라이딩은 상속받은 부모 클래스의 메서드를 자식 클래스에서 재정의하는 것입니다.<br><br>

오버로딩은 컴파일 시점에 결정되고,<br>
오버라이딩은 실행 시점(런타임) 에 어떤 메서드가 호출될지 결정된다는 차이도 있습니다.`
  },
  {
